...
//    List<OrderEntity> findAllDeliveredByMonthModified(@Param("month") int month, @Param("year") int year );
    @Query(value = "SELECT * FROM orders WHERE createdat LIKE CONCAT(:date, '%')", nativeQuery = true)
    List<OrderEntity> findAllByTime(@Param("date") String date);

    @Query(value = "SELECT * FROM orders WHERE status='DELIVERED' AND modifiedat LIKE CONCAT(:date, '%') ", nativeQuery = true)
